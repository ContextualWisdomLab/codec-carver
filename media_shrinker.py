    ...
    def command(
        ...
        *,
        ...
        if input_path is not None:
            ...
            args[input_index] = os.path.realpath(input_path)
        if output_path is not None:
            args[-1] = os.path.realpath(output_path)
...
def build_icloud_download_command(
    ...
    return [brctl_path, "download", os.path.realpath(source_path)]
...
def _copy_macos_creation_time(
    ...
    subprocess.run(
        [setfile_path, "-d", creation_date, os.path.realpath(dest)],
        check=False,
        capture_output=True,
        text=True,
    )
