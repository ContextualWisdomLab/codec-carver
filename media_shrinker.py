...
SILENCE_RE = re.compile(r"silence_(?P<type>start|end):\s*(?P<value>[0-9]+(?:\.[0-9]+)?)")
...
def parse_silencedetect_intervals(stderr: str) -> list[SilenceInterval]:
    ...
    # ⚡ Bolt Optimization:
    # We use re.finditer directly on the huge stderr string rather than calling
    # stderr.splitlines(). This avoids massive O(N) memory allocations for
    # hundreds of thousands of lines of ffmpeg log output, scanning sequentially
    # for a near-zero memory footprint and significant CPU savings.
    for match in SILENCE_RE.finditer(stderr):
        m_type = match.group("type")
        value = float(match.group("value"))
        if m_type == "start":
            current_start = value
        elif m_type == "end" and current_start is not None:
