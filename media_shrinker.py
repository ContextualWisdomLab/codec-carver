...
SILENCE_EVENT_RE = re.compile(r"silence_(?P<type>start|end):\s*(?P<value>[0-9]+(?:\.[0-9]+)?)")
...
def parse_silencedetect_intervals(stderr: str) -> list[SilenceInterval]:
    ...
    # Use re.finditer instead of stderr.splitlines() to avoid duplicating the entire
    # massive stderr string into a memory-intensive array of smaller strings.
