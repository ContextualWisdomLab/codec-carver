...
- **Upload-card interaction tree.** Empty-space clicks on `#drop-zone` and `#batch-drop-zone` open the matching picker after `DOMContentLoaded`, using `closest('input, button, label')`. Landmarks and design tokens overlay that tree; they must not replace it. Keyboard/AT stays on the native input and label. See `docs/doctoring/drop-zone-pointer-activation.md`.
