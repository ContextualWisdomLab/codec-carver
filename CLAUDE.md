...
- **`job_store.py`** — stdlib-only SQLite (WAL) durable store for async conversion jobs. The physical table is `conversion_jobs` (two-word snake_case columns). `saas_web.py` `/jobs` endpoints persist and poll through it. Callers pass `now` explicitly; the store never calls `datetime.now()` itself. A leftover one-word `jobs` table is copied forward on open. See `docs/doctoring/conversion-jobs-schema.md`.
