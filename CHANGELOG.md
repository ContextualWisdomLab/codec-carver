...
## [Unreleased]
### Added
- `<main>` 랜드마크와 `aria-controls`를 추가하여 스크린 리더 탐색 편의성을 향상했습니다.
- 입력 필드 값 변경 시 이벤트 소스 의존성을 제거해 프리셋 버튼 활성화 상태가 직접 입력 시에도 올바르게 동기화되도록 수정했습니다.
- 버튼과 호버 동작에 CSS transition을 추가해 시각적 피드백을 부드럽게 개선했습니다.
