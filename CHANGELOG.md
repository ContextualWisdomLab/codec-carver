...
### Fixed
- 단일·일괄 파일 및 대상 크기 필수 입력을 비웠을 때 `This field is required.` 안내와 custom validity·`aria-invalid` 상태를 유지하고, 유효 입력으로 돌아오면 stale required 표시를 제거합니다.
- 일괄 업로드 컨트롤을 inline listener-binding script보다 먼저 배치해 초기 DOM 파싱 중 batch listener가 존재하지 않는 요소를 역참조하지 않도록 했습니다.
