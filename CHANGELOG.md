...
### Fixed
- 단일·일괄 파일 및 대상 크기 입력이 비어 있는 경우 `This field is required.` 인라인 피드백과 custom validity·`aria-invalid` 상태를 일관되게 설정하고, 파일 선택의 `change`·`invalid`·`cancel` 경로가 같은 검증 계약을 사용하도록 수정했습니다.
