...
## 2025-02-12 - [Optimize set intersections and generator evaluation in search hot paths]
**Learning:** Using `&` on sets inside tight inner loops creates new object allocations and instantiation overhead on each iteration, which degrades performance for complex searches over large inverted indices. Furthermore, using `sum()` with a generator expression inside these loops carries generator instantiation overhead that outweighs the concise syntax benefits.
**Action:** When calculating set intersections in performance-critical code paths, initialize with `set()` and use `.intersection_update()` for subsequent operations to mutate the set in-place. Similarly, unroll `sum()` generator expressions into traditional `for` loops with explicit accumulators to bypass Python generator overhead.
