...
## 2024-05-18 - [Optimization attempt rejected: O(log N) search on silence intervals]
**Learning:** An optimization to use `bisect` and backwards iteration on `silence_intervals` for O(log N) lookup was rejected. The assumption that `silence_intervals` (sorted by `start_seconds`) allows a safe early `break` on `end_seconds` is incorrect for overlapping/nested intervals. While `silencedetect` may output non-overlapping intervals, the function signature `Iterable[SilenceInterval]` does not enforce this invariant. A proper optimization requires an interval tree, segment tree, or prefix-maximum structure, and robust property testing.
**Action:** Before optimizing search algorithms on intervals based on a sorting key (like start time), rigorously prove that the early termination conditions hold for all possible interval relations (nested, overlapping). If the invariant isn't guaranteed by the type system or input validation, don't rely on it.
## 2024-05-18 - [구간 트리(Interval) 최적화 시 조기 종료 조건 증명 필요]
**Learning:** `silence_intervals`의 O(log N) 탐색을 위해 `bisect`와 역방향 순회를 사용한 최적화 시도가 거부되었습니다. `start_seconds`로 정렬되어 있다는 사실만으로는 중첩되거나 겹치는 구간(overlapping/nested intervals)이 있을 때 `end_seconds`에 대한 안전한 조기 종료(break)를 보장할 수 없기 때문입니다. `silencedetect`가 비중첩 구간을 반환한다 하더라도, 함수 시그니처 `Iterable[SilenceInterval]`은 이러한 불변성을 보장하지 않습니다. 제대로 된 최적화를 위해서는 구간 트리, 세그먼트 트리, 혹은 prefix-maximum 구조와 함께 엄격한 속성 테스트가 필요합니다.
**Action:** 시작 시간 등 특정 키로 정렬된 구간에 대해 탐색 알고리즘을 최적화할 때는, 중첩이나 겹침 등 모든 가능한 구간 관계에 대해 조기 종료 조건이 참인지 엄밀히 증명해야 합니다. 불변성이 타입 시스템이나 입력 검증으로 보장되지 않는다면 이에 의존해서는 안 됩니다.
