...
**Learning:** In Python, `pathlib.Path.resolve()` can touch the filesystem to resolve symlinks, while `Path.absolute()` is a lexical absolute path conversion. That difference matters in hot paths when a canonical real path is not required.
**Action:** When passing argv lists to tools like `ffmpeg` or `brctl`, prefer `Path.absolute()` only when the command needs an absolute path but not symlink resolution. Passing argv as a list already avoids shell injection; absolute paths mainly avoid `resolve()` overhead and can reduce leading-hyphen option-injection risk where a tool lacks a `--` delimiter.
