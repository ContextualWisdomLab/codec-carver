...
## 2024-05-18 - [정규표현식 처리 속도 개선]
**학습:** 텍스트 토큰화 시 비단어(non-word) 문자를 제거할 때 무조건 정규표현식을 사용하면 상당한 오버헤드가 발생함.
**실행:** 순수 알파벳/숫자로만 이루어진 단어의 경우 `str.isalnum()` 검사를 선행하여 불필요한 정규표현식 엔진 호출을 방지함.
