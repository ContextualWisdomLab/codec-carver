...
## 2024-06-03 - Path Resolution Overhead in Deep Tree Traversals
**Learning:** `Path.resolve()` is extremely expensive when iterating over large directory trees, particularly if used unconditionally. Calling it thousands of times inside `os.walk` loops acts as a major performance bottleneck. Additionally, instantiating `Path` objects inside inner loops and calling methods like `.is_file()` or `.stat()` introduces object-creation overhead compared to using raw strings and `os.lstat`.
**Action:** Fast-path operations that avoid syscalls or object instantiations when not needed. Use `os.path.splitext` for quick extension filtering. Use `os.lstat` over `Path.stat()` for combined type and size checks, effectively eliminating extraneous `.is_symlink()` exceptions overhead. Conditionally defer `.resolve()` path deduplication strictly until matching exclusions are configured.
