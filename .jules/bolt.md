## 2024-06-26 - [Optimize PRAGMA journal_mode execution]
**Learning:** SQLite's `PRAGMA journal_mode=WAL` is persistent per database file. Executing it repeatedly on every short-lived connection causes redundant overhead in high-concurrency environments.
**Action:** Execute the PRAGMA once during database initialization via `conn.executescript()` alongside the schema definition.
