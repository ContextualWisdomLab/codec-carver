## 2024-08-18 - SQLite WAL pragma optimization
**Learning:** SQLite `PRAGMA journal_mode=WAL` is persistent per database file. In applications with many short-lived connections, executing it redundantly on every connection adds overhead.
**Action:** Execute it once during schema initialization via `executescript`.
