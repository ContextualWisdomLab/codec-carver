...
## 2024-06-25 - [Optimize set intersection and sum calculation in transcript search]
**Learning:** Using `candidates.intersection_update(postings)` avoids intermediate memory allocations compared to the `&` operator, and using a list comprehension inside `sum()` is faster than a generator expression due to reduced overhead in tight loops.
**Action:** Use in-place set operations and list comprehensions inside `sum()` for performance-critical hot loops.
