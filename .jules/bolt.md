...
## 2026-06-26 - [Use os.stat instead of Path.stat() in hot loops]
**Learning:** Calling `pathlib.Path(path).stat()` or `.stat()` on an existing Path object adds significant object instantiation and wrapper overhead in hot loops or batch file operations. Utilizing Python's built-in `os.stat(path)` executes measurably faster, as it natively handles path-like objects directly.
**Action:** Always prefer `os.stat()` for performance-sensitive operations where I/O efficiency is critical.
