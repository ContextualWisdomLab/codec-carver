## 2026-08-05 - [SQLite WAL Optimization]
**Learning:** SQLite's PRAGMA journal_mode=WAL is persistent per database file and does not need to be executed on every new connection.
**Action:** Execute PRAGMA journal_mode=WAL once during application initialization (e.g., alongside schema creation using conn.executescript) to avoid redundant execution overhead on every connection.
