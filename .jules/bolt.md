## 2024-07-30 - [SQLite WAL Journal Mode Persistence]
**Learning:** SQLite's `PRAGMA journal_mode=WAL` is persistent per database file. Setting it on every short-lived connection introduces redundant overhead.
**Action:** Execute the PRAGMA once during schema initialization using `conn.executescript()` instead of per connection to improve connection performance.
