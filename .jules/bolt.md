## 2024-05-24 - SQLite Persistent PRAGMA Optimization
**Learning:** SQLite's `PRAGMA journal_mode=WAL` is persistent across connections and written to the database file itself. Re-executing it on every short-lived connection wastes significant CPU and I/O overhead.
**Action:** Move persistent PRAGMA initializations to the same location where the schema is created (e.g. `__init__`) using `conn.executescript()` to safely execute multiple statements at once, rather than running them in the per-connection `_connect()` hot path.
