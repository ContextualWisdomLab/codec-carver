...
## 2024-05-18 - [Optimize _choose_silence_split_point for fast O(log N) lookup]
**Learning:** In `_choose_silence_split_point`, when looking for the latest valid split point in a list of silence intervals that are already sorted by `start_seconds`, iterating over the entire list (O(N)) using list comprehensions is very slow for large media files with thousands of intervals (e.g. taking 2.2 seconds for 100K intervals). Since the list is sorted by `start_seconds`, we can use `bisect.bisect_left` with a `key` function to perform binary search and jump directly to the relevant intervals in O(log N) time, drastically reducing lookup time.
**Action:** Always identify opportunities to use `bisect` for fast O(log N) lookups on sorted arrays instead of traversing the whole list (O(N)), especially for heavily-called internal parsing or calculation functions.
## 2024-05-18 - [O(log N) 최적화로 빠른 무음 구간 기반 분할 지점 탐색]
**Learning:** `_choose_silence_split_point` 함수 내에서, 이미 `start_seconds`에 의해 정렬되어 있는 수많은 무음 구간 리스트 중 가장 늦은 유효 분할점을 찾을 때, 리스트 전체를 순회(O(N))하는 기존 구현은 무음 구간이 수만 개인 큰 미디어 파일 처리 시 매우 느려집니다. 배열이 이미 정렬되어 있으므로, `bisect.bisect_left`를 `key` 함수와 함께 사용하여 이진 탐색을 수행하면 즉시 해당되는 구간으로 도달해 탐색 시간을 O(N)에서 O(log N)으로 비약적으로 단축할 수 있습니다.
**Action:** 내부 탐색 및 계산이 자주 호출되는 함수에서, 미리 정렬된 배열 안을 탐색할 때는 리스트 컴프리헨션(O(N)) 대신 항상 `bisect`를 통한 O(log N) 고속 탐색이 가능한지 우선적으로 식별하고 적용해야 합니다.
