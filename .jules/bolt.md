## 2026-08-02 - Optimize SQLite WAL initialization
**Learning:** SQLite's PRAGMA journal_mode=WAL is persistent per database file. In applications with many short-lived connections, executing it once during initialization rather than on every connection reduces overhead.
**Action:** Execute WAL initialization via conn.executescript() once during schema creation.
