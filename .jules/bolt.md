## 2026-08-13 - [SQLite WAL Mode Optimization]
**Learning:** SQLite `PRAGMA journal_mode=WAL` is persistent per database file. Setting it on every short-lived connection is unnecessary and incurs ~5x overhead.
**Action:** Execute it once during initialization (e.g. via `conn.executescript()` with the schema) rather than redundantly on every connection.
