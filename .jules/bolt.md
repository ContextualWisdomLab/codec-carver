## 2024-07-29 - SQLite WAL Initialization
**Learning:** SQLite WAL journaling mode is persistent per file. Running PRAGMA journal_mode=WAL on every short-lived connection is unnecessary and incurs a 2x overhead on simple query operations.
**Action:** Set WAL mode once on application startup using conn.executescript() instead of redundant PRAGMA commands per connection.
