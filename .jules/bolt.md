## 2026-06-25 - [Optimize SQLite WAL PRAGMA Execution]
**Learning:** SQLite`s `PRAGMA journal_mode=WAL` is persistent at the database file level. Executing it repeatedly on every short-lived connection (like in polling workflows) adds redundant parsing and IO overhead.
**Action:** Execute `PRAGMA journal_mode=WAL` only once during the initial schema creation phase (e.g. using `conn.executescript()`) rather than on every `connect()` invocation.
