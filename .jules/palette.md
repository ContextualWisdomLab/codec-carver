## 2024-09-01 - 명확한 빈 입력 오류 피드백
**Learning:** 숫자 입력란이 필수일 때, 사용자가 폼 내용을 비울 경우 아무런 텍스트 메시지도 없이 빨간 외곽선만 나타나면 스크린 리더 사용자나 일반 사용자 모두에게 당혹감을 줄 수 있습니다. "This field is required."와 같은 명시적인 텍스트 피드백이 필요합니다.
**Action:** 클라이언트 사이드 검증에서 값이 비어있을 때 `setCustomValidity`와 텍스트 프리뷰에 구체적인 필수 입력 누락 오류 메시지를 포함하여 명확히 안내해야 합니다.
