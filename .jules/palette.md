...
## 2024-07-28 - 숫자 입력의 빈 문자열 상태 처리
**학습:** JavaScript를 사용하여 숫자 입력에 대한 인라인 유효성 검사를 구현할 때, 빈 문자열 상태(예: `if (this.value === '')`)를 명시적으로 처리하지 않으면 네이티브 HTML5 제약 조건(예: `required`)이 적용되기 전에 이전의 잘못된 상태(예: 음수)가 유지되어 문제가 발생합니다.
**실행:** 인라인 유효성 검사 로직 내에서 빈 문자열 상태를 먼저 확인하고, 시각적 텍스트를 지우는 것과 함께 `this.setCustomValidity('')` 및 `this.removeAttribute('aria-invalid')`를 호출하여 상태를 완전히 초기화해야 합니다.
