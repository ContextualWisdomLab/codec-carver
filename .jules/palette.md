...
## 2024-08-16 - 브라우저 네이티브 한계 검증과 접근성 상태 동기화
**학습:** 숫자 입력 필드에 `max` 제약 조건을 추가하더라도 사용자가 직접 숫자를 입력할 때 자바스크립트의 커스텀 검증 로직이 화면 리더기에게 적절한 한계 초과 안내를 제공하지 않으면 접근성이 저하됩니다.
**실행:** 항상 HTML5 제약 조건(`max`, `min` 등)과 연동하여 자바스크립트 검증에서 한계 초과 시 `setCustomValidity` 및 `aria-invalid` 속성을 활용해 접근성 높은 에러 메시지를 제공해야 합니다.
