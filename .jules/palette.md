...
## 2024-07-28 - 숫자 입력 필드의 빈 문자열 상태 처리 및 이벤트 리스너 최적화
**학습:** 숫자 입력을 위한 JavaScript 인라인 검증을 구현할 때, 빈 문자열 상태를 명시적으로 처리하지 않으면(예: `if (this.value === '')`) 시각적인 텍스트가 지워져도 브라우저 내부적으로는 이전에 입력된 잘못된 상태(예: 음수)를 유지하게 되어 접근성 및 사용성에 심각한 영향을 미칩니다. 또한, 빈번하게 발생하는 `input` 이벤트 내부에서 매번 DOM 요소를 쿼리하고 속성을 파싱하면 UI 성능이 저하됩니다.
**실행:** 빈 문자열 입력 시 `setCustomValidity('')` 및 `removeAttribute('aria-invalid')`를 조건문 내에서 명시적으로 호출하여 사용자 정의 오류를 지우고, 잦은 이벤트 리스너 외부에서 NodeList와 파싱된 값을 미리 계산하고 캐싱하여 성능과 사용성을 동시에 최적화해야 합니다.
