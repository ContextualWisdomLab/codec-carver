...
## 2026-09-16 - Client-side file upload 3-state validation
**Learning:** The HTML accept attribute is easily bypassed. Furthermore, browsers sometimes omit the MIME type (empty file.type).
**Action:** Implement a three-state UX: accept known valid types, block known invalid types (using setCustomValidity and aria-invalid), and explicitly warn without blocking on unknown types.
