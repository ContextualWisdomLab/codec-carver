...
## 2026-09-12 - Client-side file upload validation against file.type
**Learning:** Implementing client-side file upload validation against `file.type` must account for browsers sometimes omitting the MIME type. Implementing a three-state UX (accept valid, block invalid, warn on unknown) is necessary to avoid blocking valid edge cases while still catching obvious errors early.
**Action:** Always implement a three-state UX for file type validation: accept known valid types, block known invalid types using `setCustomValidity` and `aria-invalid`, and explicitly warn (without blocking) on unknown types (empty `file.type`).
