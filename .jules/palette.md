...
## 2024-08-05 - 프리셋 버튼의 수동 입력 동기화
**학습:** 입력 필드 값에 따라 프리셋 버튼의 활성화 상태(`aria-pressed`)를 업데이트할 때, `e.isTrusted` 이벤트 속성을 통해 수동 입력을 필터링하면 사용자가 직접 프리셋과 동일한 값을 타이핑했을 때 UI가 일치하는 상태를 반영하지 못합니다.
**실행:** 입력 필드 값과 프리셋 버튼의 동기화는 입력 방식(자동/수동)과 관계없이 항상 입력 필드의 현재 값을 기준으로 평가하여 즉각적이고 일관된 상태 피드백을 제공해야 합니다.
