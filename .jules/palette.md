...
## 2024-09-19 - 숫자 입력 필드의 badInput 검증 개선
**Learning:** 숫자 입력 필드에 잘못된 형식의 문자(예: 'e')를 입력하면 브라우저에서 `this.value`가 빈 문자열로 처리되지만 `this.validity.badInput`은 true가 됩니다. 이전에는 빈 문자열일 때 검증 오류를 모두 무시하여, 잘못된 문자를 입력해도 폼 제출은 막히지만 오류 메시지가 나타나지 않아 사용자가 혼란스러워했습니다.
**Action:** 빈 문자열을 처리할 때 `this.validity.badInput`을 확인하여 잘못된 입력 형식에 대해 즉각적인 인라인 오류 메시지를 표시해야 합니다.
