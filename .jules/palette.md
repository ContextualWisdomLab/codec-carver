...
## 2024-08-30 - HTML5 Constraints and JS Validation
**Learning:** Adding HTML5 constraints like `max` to inputs is good, but without mirroring that logic in client-side JavaScript via `setCustomValidity` and `aria-invalid`, users who paste or programmatically enter invalid data won't receive the necessary accessible inline feedback. Stepper arrows enforce limits, but typing doesn't.
**Action:** Always complement HTML5 numeric input constraints with corresponding JavaScript validation logic that sets `setCustomValidity` and updates `aria-invalid` to ensure accessible error states are correctly surfaced for all interaction methods.
