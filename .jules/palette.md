...
## 2024-07-17 - 일괄 업로드 폼 총 파일 용량 클라이언트 측 유효성 검사
**Learning:** 일괄 파일 업로드 시 총 용량이 서버의 허용 제한(5 GiB)을 초과하는 경우, 사용자가 폼을 제출하고 서버의 오류 응답을 받을 때까지 기다려야 하므로 시간과 대역폭이 낭비되고 사용자 경험이 저하되었습니다.
**Action:** 일괄 파일 업로드 폼의 파일 선택 이벤트 핸들러(`onchange`)에 총 파일 용량을 계산하고 제한을 초과할 경우 즉각적으로 `setCustomValidity`와 `aria-invalid`를 설정하여 빨간색 에러 메시지를 표시하는 클라이언트 측 유효성 검사 로직을 추가해야 합니다.
