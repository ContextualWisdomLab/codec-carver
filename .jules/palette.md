...
## 2026-09-13 - Three-state UX for File Upload Validation
**Learning:** When implementing client-side file upload validation against `file.type`, the HTML `accept` attribute is easily bypassed. Implementing a three-state UX—accept known valid types, block known invalid types using `setCustomValidity` and `aria-invalid`, and explicitly warn (without blocking) on unknown types (empty `file.type`)—provides robust, accessible feedback as browsers may sometimes omit the MIME type.
**Action:** Always implement a three-state UX (accept, block with aria-invalid/setCustomValidity, warn) when validating `file.type` client-side to handle edge cases where browsers omit MIME types.
