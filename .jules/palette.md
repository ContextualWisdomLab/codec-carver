...
## 2026-09-20 - 숫자 입력 필드의 유효하지 않은 입력 처리
**Learning:** HTML `<input type="number">`에서 유효하지 않은 문자('e', '-' 등)를 입력하면 브라우저는 `this.value`를 빈 문자열로 평가하지만 `this.validity.badInput`을 true로 설정합니다. 커스텀 유효성 검사 로직에서 `badInput`을 확인하지 않으면 필드가 비워진 것으로 오해하여 유효성 검사 오류를 무시하게 됩니다.
**Action:** 숫자 입력 필드에 대한 클라이언트 측 유효성 검사 시 항상 `this.validity.badInput`을 확인하여 유효하지 않은 입력에 대해 즉각적인 오류 피드백(빈 문자열로 오인하지 않도록)을 제공해야 합니다.
