...
## 2024-08-04 - 필수 숫자 입력의 빈 문자열 검증
**Learning:** `required` 숫자 입력의 빈 문자열은 정상 상태가 아니라 누락된 필수 값입니다. 이전 custom validity나 표시 스타일을 먼저 정리하더라도 빈 값 자체에는 명시적인 오류 메시지와 `aria-invalid="true"`가 남아야 하며, 유효한 값으로 돌아왔을 때만 해당 오류 상태를 해제해야 합니다.
**Action:** 필수 숫자 입력의 빈 문자열 분기에서는 `This field is required.`를 `setCustomValidity`와 인라인 오류 영역에 설정하고 `aria-invalid="true"`를 유지합니다. 재검증 시작 시 stale required 스타일을 제거한 뒤 현재 값에 따라 상태를 다시 결정합니다.
