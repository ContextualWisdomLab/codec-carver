...
## 2024-05-18 - 숫자 입력란 빈 문자열(Empty String)에 대한 인라인 유효성 검사 리셋

**학습:** HTML5의 내장 제약 조건(예: required)이 동작하기 전에 브라우저 내부적으로 이전의 유효하지 않은 상태(예: 음수 입력 등)를 보존하고 있을 수 있기 때문에, JavaScript로 숫자 입력을 처리할 때 입력란이 비워진 상태(`this.value === ''`)를 명시적으로 처리해야 함을 배웠습니다. 시각적으로 텍스트가 지워지더라도 브라우저는 에러 상태를 기억하고 있어, 수동으로 오류 상태와 커스텀 유효성 메시지를 초기화하지 않으면 사용자에게 혼란을 줄 수 있습니다.

**실행:** 인라인 폼 유효성 검사 로직 구현 시 항상 `if (this.value === '')` 블록을 먼저 체크하도록 작성하고, 그 안에서 `this.setCustomValidity('')` 및 `this.removeAttribute('aria-invalid')`를 호출하여 이전 에러 상태를 안전하게 해제하도록 적용하겠습니다.
