...
## 2024-06-30 - aria-invalid 시각적 피드백 동기화
**Learning:** JavaScript를 통해 클라이언트 측 유효성 검사 중 입력 필드에 `aria-invalid="true"` 속성을 동적으로 설정할 때, 해당 접근성 속성과 일치하는 즉각적인 시각적 피드백이 시각 장애가 없는 사용자에게도 제공되어야 합니다.
**Action:** 폼 검증 시 화면 판독기 사용자를 위해 `aria-invalid` 상태를 동적으로 토글할 때, 시각적으로도 명확한 피드백(예: `input[aria-invalid="true"] { border-color: #dc3545; outline: 2px solid #dc3545; }`)을 반드시 제공해야 합니다.
