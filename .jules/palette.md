...
## 2026-09-21 - 파일 업로드 시 클라이언트 측 타입 검증\n**Learning:** 드래그 앤 드롭 영역이나 일반 파일 입력(input)에서 HTML `accept` 속성은 잘못된 파일 선택을 완벽히 막지 못하며, 잘못된 파일이 폼 제출로 이어져 사용자가 에러를 뒤늦게 인지하는 문제가 발생합니다.\n**Action:** 항상 클라이언트 측 자바스크립트에서 `file.type`을 검증하고, 실패 시 `setCustomValidity()` 및 `aria-invalid`를 활용해 접근성 높은 인라인 에러 피드백을 폼 제출 전에 즉시 제공합니다.
