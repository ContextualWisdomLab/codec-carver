...
## 2026-09-22 - Drag-and-drop File Type & HTML5 Number Validation
**Learning:** The HTML accept attribute does not prevent invalid files from being dropped. Additionally, typing invalid characters (e.g., 'e') in an <input type="number"> sets the value to empty string ('') while badInput becomes true. Custom validation must explicitly check file.type and validity.badInput.
**Action:** Implement explicit file.type validation for drop zones and check validity.badInput before assuming an empty numeric input field is merely cleared by the user.
