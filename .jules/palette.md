...
## 2024-05-18 - 입력란 비우기 상태에 대한 인라인 유효성 검사 개선
**학습:** 숫자 입력 필드에서 값을 완전히 지웠을 때(빈 문자열 상태) 명시적으로 처리하지 않으면, 브라우저는 이전에 입력된 잘못된 상태(예: 음수)를 내부적으로 유지하게 되어, HTML5의 `required`와 같은 기본 제약 조건이 제대로 작동하기 전에 사용자가 혼란을 겪을 수 있습니다.
**실행:** 인라인 유효성 검사 이벤트 리스너 내에서 `if (this.value === '')` 조건을 추가하여 빈 상태를 감지하고, 이 블록 안에서 `this.setCustomValidity('')` 및 `this.removeAttribute('aria-invalid')`를 명시적으로 호출하여 오류 상태를 초기화하도록 합니다.
