...
## 2024-09-07 - 드래그 앤 드롭 파일 타입 유효성 검사
**Learning:** HTML accept 속성은 드래그 앤 드롭 시 잘못된 파일이 추가되는 것을 완벽히 막아주지 못합니다.
**Action:** 항상 file.type을 통해 명시적인 클라이언트 사이드 JavaScript 유효성 검사를 추가하고, setCustomValidity와 aria-invalid를 사용해 접근성 피드백을 인라인으로 제공해야 합니다.
