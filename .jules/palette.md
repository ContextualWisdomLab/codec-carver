...
## 2024-05-18 - 숫자 입력 필드 인라인 유효성 검사 UX 개선
**학습:** 숫자 입력을 위한 인라인 유효성 검사를 구현할 때, 값이 비워진(empty string) 상태를 명시적으로 처리하지 않으면 브라우저가 사용자에게 'Must be greater than 0.'과 같은 불필요한 커스텀 에러를 표시하고 HTML5의 기본 `required` 속성 동작을 방해하게 됩니다.
**실행:** 입력값이 `this.value === ''`일 때, 명시적으로 `this.setCustomValidity('')` 및 `this.removeAttribute('aria-invalid')`를 호출하여 커스텀 에러를 지우고 네이티브 제약 조건이 정상적으로 동작하도록 해야 합니다.
