...
## 2024-08-11 - 대상 바이트 입력 필드 최대값 검증 추가
**학습:** 서버에서 허용하는 최대 파일 크기 제한이 있음에도 프론트엔드의 대상 바이트 숫자 입력 필드에 HTML5 `max` 속성 및 클라이언트 측 검증이 없어, 사용자가 허용치를 초과하는 값을 입력하고 제출할 때까지 오류를 인지하지 못하는 사용성 문제가 있었습니다.
**실행:** 숫자 입력 필드에 `max` 속성을 추가하고, JavaScript `input` 이벤트 리스너에서 입력값이 최대값을 초과하는지 검사하여 초과 시 즉각적인 시각적 피드백과 함께 `setCustomValidity` 및 `aria-invalid`를 설정해야 합니다.
