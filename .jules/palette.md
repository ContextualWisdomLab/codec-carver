...
## 2024-07-21 - 입력 유효성 검사를 위한 빈 문자열 처리
**Learning:** 숫자 입력 필드에 대한 JS 유효성 검사에서 사용자가 텍스트를 완전히 지워 빈 문자열이 되더라도 이전의 에러 상태(시각적 및 접근성)가 남아있는 문제가 있습니다. 입력값이 빈 문자열일 때를 명시적으로 처리하여 유효성 검사 상태를 리셋해야 합니다.
**Action:** 항상 네이티브 HTML 입력 요소의 JS 유효성 검사 시 `if (this.value === '')`와 같은 조건을 추가하여 `aria-invalid` 속성 제거 및 `setCustomValidity('')`를 호출해 커스텀 유효성 상태를 명확히 지워주세요.
