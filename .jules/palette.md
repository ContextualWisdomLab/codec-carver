...
**Action:** 드롭 존 클릭 리스너는 `e.target.closest('input, button, label')`로 중첩 컨트롤을 제외한다. `tagName`만 보면 LABEL 안의 required-star SPAN이 피커를 두 번 연다. `role="region"` 랜드마크는 같은 클릭/드래그 트리 위에 겹친다. 리스너를 지우거나 카드를 `role="button"`으로 바꾸지 않는다.
