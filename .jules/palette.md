...
## 2024-07-21 - [숫자 입력의 빈 문자열 상태 처리]
**Learning:** 숫자 입력을 위한 JavaScript 인라인 유효성 검사 구현 시, 사용자가 값을 모두 지워 빈 문자열("")이 된 상태를 명시적으로 처리해야 합니다. 특히, 이 조건 블록 내에서 `this.setCustomValidity('')`와 `this.removeAttribute('aria-invalid')`를 호출해야 합니다. 시각적으로 텍스트가 지워지더라도, `required`와 같은 기본 HTML5 제약 조건이 적용되기 전에 명시적으로 리셋하지 않으면 브라우저 내부적으로는 이전에 입력된 유효하지 않은 상태(예: 음수)를 유지할 수 있습니다.
**Action:** 숫자 입력에 대한 `input` 이벤트 리스너를 작성할 때, 파싱 및 유효성 검사 로직을 실행하기 전에 항상 `if (this.value === '')`를 체크하고, 커스텀 에러 상태(`setCustomValidity`)와 관련 ARIA 속성(`aria-invalid`)을 초기화하도록 적용합니다.
