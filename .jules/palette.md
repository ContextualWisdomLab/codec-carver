...
## 2024-07-25 - 빈 숫자 입력 필드의 인라인 유효성 검사 처리 개선
**Learning:** 숫자 입력 필드에서 JavaScript로 인라인 유효성 검사를 구현할 때, 사용자가 값을 지워 빈 문자열 상태가 되었을 때 커스텀 오류를 명시적으로 해제하지 않으면 HTML5의 `required` 기본 제약 조건이 제대로 작동하기 전에 이전의 유효하지 않은 상태(예: 음수)가 보이지 않게 남아 문제가 발생할 수 있음을 확인했습니다.
**Action:** 항상 `if (this.value === '')`와 같은 빈 문자열 상태를 명시적으로 확인하는 조건문을 추가하고, 그 블록 안에서 `this.setCustomValidity('')` 및 `this.removeAttribute('aria-invalid')`를 호출하여 커스텀 유효성 검사 상태를 완벽히 초기화해야 합니다.
