...
## 2024-07-30 - 숫자 입력의 빈 문자열 상태 처리 시 UX 및 접근성 개선
**학습:** 숫자 입력(numeric inputs)에 대해 인라인 JavaScript 유효성 검사를 수행할 때, 사용자가 값을 지워 빈 문자열(`''`)이 된 경우를 명시적으로 처리하지 않으면 이전에 입력된 유효하지 않은 상태(예: 음수)의 커스텀 에러 메시지가 계속 남게 되어 사용자에게 혼란을 주고 네이티브 HTML5 `required` 제약 조건의 동작을 방해합니다.
**실행:** 인라인 유효성 검사 로직에 빈 문자열(`if (this.value === '')`) 상태를 가장 먼저 확인하는 조건문을 추가하고, 그 안에서 `this.setCustomValidity('')` 및 `this.removeAttribute('aria-invalid')`를 호출하여 시각적 오류 상태와 내부 오류 상태를 모두 명시적으로 초기화해야 정합성 및 접근성이 개선됩니다.
