...
## 2026-09-17 - Client-side Maximum Value Validation
**Learning:** Missing client-side maximum value validations for numeric inputs forces users to rely on backend errors for easily preventable mistakes, leading to a poor UX.
**Action:** Always mirror backend maximum limits (e.g., MAX_UPLOAD_BYTES) in client-side inline validation using formatBinaryBytes for clear, immediate feedback alongside setCustomValidity and aria-invalid.
