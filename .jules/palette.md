...
## 2024-09-15 - Semantic Document Structure (main and h1)
**Learning:** Missing `<main>` and `<h1>` tags severely impact document structure and screen reader navigation. Users rely on top-level headings and the main landmark to orient themselves and skip repetitive content.
**Action:** Always include a single semantic `<main>` tag wrapping the primary content and at least one `<h1>` tag at the top of the content for proper accessibility landmarks.
