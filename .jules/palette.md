...
## 2024-07-24 - 숫자 입력 필드의 빈 문자열 상태 처리
**학습:** 숫자 입력 필드에서 JavaScript로 인라인 유효성 검사를 구현할 때, 빈 문자열(`''`) 상태를 명시적으로 처리하여 사용자 지정 오류를 지워야 합니다. 시각적 텍스트가 지워지더라도 브라우저는 명시적으로 재설정하지 않는 한, 기본 HTML5 제약 조건(예: `required`)이 적용되기 전에 이전에 입력된 잘못된 상태(예: 음수)를 내부적으로 유지합니다.
**실행:** 인라인 유효성 검사 내에서 `this.value === ''` 조건을 추가하고 해당 블록 안에서 `this.setCustomValidity('')` 및 `this.removeAttribute('aria-invalid')`를 명시적으로 호출하여 오류 상태를 초기화합니다.
