...
## 2024-05-24 - 숫자 입력 필드의 빈 문자열 상태 처리 (UX 개선)
**학습:** HTML5 `<input type="number">`에서 사용자가 값을 모두 지워 빈 문자열(`''`)이 되었을 때, 시각적으로는 텍스트가 지워지더라도 명시적으로 재설정하지 않으면 브라우저 내부에 이전의 잘못된 상태(예: 음수)가 유지되어 `required`와 같은 기본 제약 조건 처리를 방해할 수 있다.
**실행:** 인라인 유효성 검사를 구현할 때 `if (this.value === '')` 블록을 통해 `this.setCustomValidity('')` 및 `this.removeAttribute('aria-invalid')`를 명시적으로 호출하여 오류 상태를 깨끗이 초기화해야 전제 오류 조건을 방지할 수 있다.
