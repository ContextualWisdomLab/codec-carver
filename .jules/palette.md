...
## 2024-05-30 - Client-side validation for file inputs
**Learning:** The HTML `accept="audio/*,video/*"` attribute filters the file picker but does not prevent users from dropping invalid files via drag-and-drop. Explicit JavaScript validation is required to provide immediate UX feedback.
**Action:** Always add explicit client-side JavaScript validation and error messaging for file types on file inputs, even when `accept` is used.
