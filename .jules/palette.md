## 2024-10-24 - Required field validation feedback
**Learning:** Clearing a required form input should not silently remove validation feedback. Providing an explicit inline error message and setting `aria-invalid="true"` ensures screen readers and sighted users receive clear feedback about the missing required state.
**Action:** Always set a clear error message (e.g., 'This field is required.') and `aria-invalid="true"` when a required input becomes empty, rather than just clearing the preview text.
