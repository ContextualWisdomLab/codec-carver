...
## 2026-09-20 - Semantic document structure and page-title ownership
**Learning:** A single balanced `<main>` and one top-level `<h1>` provide document orientation, while visual title requirements need an owned component style rather than an unverified prose claim.
**Action:** Keep exactly one balanced main landmark and one `<h1 class="page-title">`; validate the component style in maintained tests and require current-head browser/accessibility-tree evidence before claiming delivered accessibility or responsive layout.
