...
## 2024-05-18 - `hmac.compare_digest` 및 Non-ASCII 문자로 인한 500 에러
**결함:** `hmac.compare_digest`에 Non-ASCII 문자가 포함된 문자열을 전달할 경우 `TypeError`가 발생하여 해당 인증 요청이 500 Server Error로 종료됩니다.
**학습:** `hmac.compare_digest`는 ASCII-only `str` 비교를 지원하지만, Non-ASCII 문자가 포함된 `str` 비교는 `TypeError`를 발생시킵니다.
