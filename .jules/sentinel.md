## 2026-07-16 - [Sentinel: Path Traversal in File Download / Zip Slip]
**Vulnerability:** Download Path Traversal / Zip Slip (CWE-22) via backslash directory separators in uploaded filenames and URLs.
**Learning:** When downloading files or deriving safe filenames from user input (e.g., `UploadFile.filename` or URL paths), relying on `pathlib.Path(filename).name` or `path.split('/')[-1]` is insufficient on POSIX systems (Linux). On POSIX, the backslash (`\`) is treated as a regular character, not a directory separator. If an attacker submits a filename like `..\..\..\startup.bat`, the basename extraction fails to sanitize it, and the output path or `FileResponse` filename will retain the traversal sequence. This can cause vulnerable Windows clients or extraction tools to write the file outside the intended directory.
**Prevention:** Explicitly normalize backslashes to forward slashes (e.g., `filename.replace('\', '/')`) before attempting to extract the basename, ensuring traversal payloads are stripped universally regardless of the backend OS.
## 2026-07-16 - [Sentinel: False Positive Argument Injection in Plan Templates]
**Vulnerability:** Argument Injection via relative paths starting with a hyphen in `media_shrinker.py` `ConversionPlan` templates.
**Learning:** In `media_shrinker.py`, the relative paths stored in the immutable `ConversionPlan.ffmpeg_args` template (e.g. `str(source_path)`) are never sent to ffmpeg in production. The method `ConversionPlan.command()` explicitly replaces these placeholders with absolute `.resolve()` values before passing them to `subprocess.run`, neutralizing any argument injection risk. Modifying the templates directly duplicates existing protections.
**Prevention:** Future security findings must trace attacker input to the exact executed argv and demonstrate the failure at the actual sink before changing templates or declaring a vulnerability.
