...
## 2024-05-24 - hmac.compare_digest Non-ASCII DoS
**Vulnerability:** `hmac.compare_digest()` crashes with a `TypeError` when comparing strings containing non-ASCII characters. Since `x-api-key` is user input, this causes an unhandled exception and a Denial of Service.
**Learning:** Python's `hmac.compare_digest` does not support non-ASCII string comparisons.
**Prevention:** Always encode user-controlled input and secrets to bytes (`.encode('utf-8')`) before using `hmac.compare_digest`.
