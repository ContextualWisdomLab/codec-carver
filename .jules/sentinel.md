...
**Research basis:** Crosby, S. A., & Wallach, D. S. (2003). Denial of service via algorithmic complexity attacks. In *12th USENIX Security Symposium (USENIX Security 03)*. USENIX Association. https://www.usenix.org/conference/12th-usenix-security-symposium/denial-service-algorithmic-complexity-attacks
**Relevance:** The paper demonstrates that attacker-controlled inputs can amplify server-side work and exhaust finite resources even with low request volume. This upload boundary therefore validates before allocation and guarantees cleanup when archive creation fails.
