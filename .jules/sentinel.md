...
## 2026-09-24 - Python hmac.compare_digest TypeError DoS
**Vulnerability:** Unhandled TypeError when processing non-ASCII characters in API key headers.
**Learning:** Python's `hmac.compare_digest` throws a TypeError when comparing strings with non-ASCII characters, which leads to Application crashes or Denial of Service when validating potentially untrusted strings (like API keys from HTTP headers).
**Prevention:** Always encode strings to bytes (e.g., `.encode('utf-8')`) before comparison using `hmac.compare_digest`.
