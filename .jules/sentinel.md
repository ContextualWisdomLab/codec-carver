...
## 2024-05-18 - `hmac.compare_digest` 및 Non-ASCII 문자로 인한 500 에러 (DoS)
**취약점:** `hmac.compare_digest`에 Non-ASCII 문자가 포함된 문자열을 전달할 경우 `TypeError`가 발생하여 500 Server Error가 발생합니다. 악의적인 사용자가 이를 악용해 서비스 거부(DoS) 공격을 시도할 수 있습니다.
**학습:** `hmac.compare_digest`는 Python에서 유니코드 문자열 간 비교를 지원하지 않습니다.
**예방:** `hmac.compare_digest`를 호출할 때 인자로 전달되는 문자열을 항상 `encode('utf-8')`을 통해 바이트 문자열로 변환한 후 비교해야 합니다.
