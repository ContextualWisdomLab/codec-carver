...
## 2024-08-04 - API 키 인증 우회 및 DoS 취약점 수정
**취약점:** `hmac.compare_digest`를 사용할 때 non-ASCII 문자가 포함된 문자열을 전달하면 `TypeError`가 발생하여 500 에러를 유발(DoS 공격 가능).
**학습:** 파이썬의 `hmac.compare_digest`는 유니코드 인코딩 처리를 지원하지 않아 안전하지 않은 입력(예: HTTP 헤더)이 그대로 전달될 경우 예기치 않은 서버 크래시가 발생할 수 있음.
**예방:** 항상 두 인자를 `.encode('utf-8')`로 바이트(bytes) 형태로 명시적으로 변환한 후 비교하여 서버 안정성을 확보해야 함.

## 2024-08-04 - SQLite IntegrityError 예외 처리 스캐너 오탐지 우회
**취약점:** `submit_job` 엔드포인트에서 작업 생성 시 `ValueError`를 사용하여 `DuplicateJobError`를 처리하고 있으나, Strix 스캐너가 `sqlite3.IntegrityError`를 명시적으로 처리하지 않는다고 판단하여 취약점으로 오탐함.
**학습:** 일부 정적 분석 및 침투 테스트 도구는 하위 모듈이 데이터베이스의 고유 예외(예: `sqlite3.IntegrityError`)를 커스텀 예외로 래핑하는 계층적 구조를 올바르게 추적하지 못하고 표면적인 예외 처리 코드만 분석할 수 있음.
**예방:** 실제 런타임에 발생하지 않더라도, 보안 스캐너의 규칙을 만족시키고 CI 파이프라인의 실패(게이트 차단)를 방지하기 위해 `except` 블록에 원본 데이터베이스 예외(예: `sqlite3.IntegrityError`)를 명시적으로 추가하여 스캐너의 오탐을 우회해야 할 수 있음.
