...
## 2026-07-15 - [Sentinel: FastAPI hmac.compare_digest DoS]
**Vulnerability:** Denial of Service (DoS) via unhandled TypeError when comparing non-ASCII API keys.
**Learning:** Python's `hmac.compare_digest` raises a TypeError if provided with strings containing non-ASCII characters, which can crash the application or endpoint if untrusted input (like HTTP headers) is not validated or encoded first.
**Prevention:** Always encode potentially untrusted strings to bytes (e.g., using `.encode('utf-8')`) before passing them to `hmac.compare_digest`.
