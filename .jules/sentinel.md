...
## 2024-09-02 - hmac.compare_digest DoS Vulnerability
**Vulnerability:** `hmac.compare_digest` throws a `TypeError` when comparing strings with non-ASCII characters, which could lead to an unhandled exception and Denial of Service (DoS) when used with user-provided HTTP headers like API keys.
**Learning:** Python's `hmac.compare_digest` does not support non-ASCII string comparisons.
**Prevention:** Always encode strings to bytes (e.g., `.encode('utf-8')`) before passing them to `hmac.compare_digest`, especially when the input is user-controlled.
