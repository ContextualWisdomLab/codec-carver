    ...
    <div class="box">
        ...
        <form action="/shrink" method="post" enctype="multipart/form-data" id="shrink-form">
            <p>
                ...
                <input type="file" id="file" name="file" accept="audio/*,video/*" aria-describedby="file_help file_size_preview" required onchange="const f = this.files[0]; const preview = document.getElementById('file_size_preview'); if(f) { const val = f.size; if (val > 5368709120) { this.setCustomValidity('File exceeds 5 GB limit'); preview.innerText = 'Error: File exceeds 5 GB limit'; preview.style.color = '#dc3545'; } else { this.setCustomValidity(''); let text = ''; if (val < 1000) text = val + ' B'; else if (val < 1000000) text = (val / 1000).toFixed(2) + ' KB'; else if (val < 1000000000) text = (val / 1000000).toFixed(2) + ' MB'; else text = (val / 1000000000).toFixed(2) + ' GB'; preview.innerText = 'Selected file size: ' + text; preview.style.color = '#17a2b8'; } } else { this.setCustomValidity(''); preview.innerText = ''; }">
